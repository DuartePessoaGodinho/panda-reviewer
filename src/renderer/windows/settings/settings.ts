...
let selectedIde: 'vscode' | 'intellij' = 'vscode';
let selectedAiProvider: AiReviewProvider = 'claude';
...
function renderPaths(): void {
  ...
  list.querySelectorAll('input[data-index]').forEach(el => {
    (el as HTMLInputElement).addEventListener('input', e => {
      ...
      paths[idx] = (e.target as HTMLInputElement).value;
    });
  });

  list.querySelectorAll('[data-browse]').forEach(el => {
    el.addEventListener('click', async () => {
      const idx = parseInt((el as HTMLElement).dataset.browse!);
      const folder = await window.api.pickFolder();
      if (folder) { paths[idx] = folder; renderPaths(); }
    });
  });
...
function selectIde(ide: 'vscode' | 'intellij'): void {
  selectedIde = ide;
  ...
  (document.getElementById('intellijPathRow') as HTMLElement).style.display =
    ide === 'intellij' ? 'block' : 'none';
}

function selectAiProvider(provider: AiReviewProvider): void {
...
function setAiReviewEnabled(enabled: boolean): void {
  aiReviewEnabled = enabled;
  (document.getElementById('aiReviewEnabled') as HTMLInputElement).checked = enabled;
  document.getElementById('aiProviderGrid')!.classList.toggle('disabled', !enabled);
}

function setTokenStatus(state: 'loading' | 'ok' | 'err' | 'hidden', msg = ''): void {
...
function updateIntervalLabel(mins: number): void {
  document.getElementById('intervalLabel')!.textContent = `${mins} min`;
}

async function init(): Promise<void> {
  const settings = await window.api.getSettings();
  (document.getElementById('token') as HTMLInputElement).value = settings.gitlabToken ?? '';
  paths = [...(settings.repoPaths ?? [])];
  intellijPath = settings.intellijPath ?? '';
  (document.getElementById('intellijPath') as HTMLInputElement).value = intellijPath;
  pollingIntervalMins = Math.round((settings.pollingIntervalMs ?? 300000) / 60000);
  const slider = document.getElementById('pollingInterval') as HTMLInputElement;
  ...
  if (settings.gitlabToken) setTokenStatus('ok', `Token saved`);
}
...
document.getElementById('token')!.addEventListener('input', () => {
  ...
  tokenDebounce = setTimeout(async () => {
    ...
    if (!token) { setTokenStatus('hidden'); return; }
    try {
...
document.getElementById('addPathBtn')!.addEventListener('click', async () => {
  const folder = await window.api.pickFolder();
  if (folder) { paths.push(folder); renderPaths(); }
});

document.getElementById('pollingInterval')!.addEventListener('input', (e) => {
  pollingIntervalMins = parseInt((e.target as HTMLInputElement).value);
  updateIntervalLabel(pollingIntervalMins);
});
...
document.getElementById('aiReviewEnabled')!.addEventListener('change', (e) => {
  setAiReviewEnabled((e.target as HTMLInputElement).checked);
});

document.getElementById('intellijPath')!.addEventListener('input', (e) => {
  intellijPath = (e.target as HTMLInputElement).value;
});
...
document.getElementById('browseIntelliJBtn')!.addEventListener('click', async () => {
  ...
  if (file) {
    intellijPath = file;
    (document.getElementById('intellijPath') as HTMLInputElement).value = file;
  }
});

document.getElementById('tokenHelpLink')!.addEventListener('click', () =>
